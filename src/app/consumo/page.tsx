...
import { ShieldCheck, ArrowLeft, Loader2, Plus, Droplets, AlertTriangle, CheckCircle2, FileDown, Search, Filter, X, Edit, Info } from 'lucide-react';
...
export default function ConsumoAguaPage() {
    ...
    const atualizarPreview = async () => {
        // 1. Busca leitura anterior no banco
        ...
        // 2. CORREÇÃO NA COMPARAÇÃO DE DATAS DO PREVIEW
        const getCompetencia = (dateStr: string) => dateStr ? dateStr.substring(0, 7) : null;

        const mesAtual = getCompetencia(formData.data_leitura);
        const mesAnterior = anteriorObj ? getCompetencia(anteriorObj.data_leitura) : null;

        // Se não tem anterior OU o mês mudou, então é o Primeiro Mês (Consumo Zero)
        ...
        if (isPrimeiroMes) {
            consumo = 0; // Regra: Primeiro registro do mês zera o consumo
        } else {
        ...
        // Só excede meta se NÃO for a leitura inicial (pois inicial é sempre 0)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Validação extra (exceto se for primeiro do mês)
    ...
    const handleExportPDF = () => {
        ...
        const tableData = registros.map(r => [
            ...
            r.leitura_anterior,
            r.leitura_atual,
            r.consumo_dia === 0 ? '0 (Inicial)' : `${r.consumo_dia.toFixed(0)} m³`,
                            ...
                            {registros.map(reg => (
                                <tr key={reg.id} className="hover:bg-slate-50">
                                    ...
                                    <td className="p-4 font-bold text-lg">
                                        {reg.consumo_dia === 0 ? <span className="text-blue-500 text-xs">0 (Início)</span> : `${reg.consumo_dia.toFixed(0)} m³`}
                                    </td>
